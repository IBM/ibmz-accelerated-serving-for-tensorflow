#!/bin/bash
# Prerequisites for the credit-card-fraud serving sample.
#
# Usage:
#   ./prerequisites.sh <tf-base-image> [/path/to/card_transaction.v1.csv]
#
...
# deployment script. The TensorFlow Serving container is started separately
# after deployment — see README.md for the full workflow.
...
if [[ -z "${BASE_IMAGE}" ]]; then
    ...
    echo "Usage: ./prerequisites.sh <tf-base-image> [/path/to/card_transaction.v1.csv]" >&2
    exit 1
fi
...
IMAGE_TAG="ccf-serving-sample:latest"
