...
- [Overview](#overview)
- [Downloading the IBM Z Accelerated Serving for TensorFlow container image](#container)
- [Container Image Contents](#contents)
- [TensorFlow Serving Usage](#tensorflow-serving)
...
[TensorFlow Serving](https://www.tensorflow.org/) is an open source,
high-performance, serving system that provides a system to handle the
**inference** aspect of machine learning.
...
requires credentials for the IBM Z and LinuxONE Container Registry,
[icr.io](https://icr.io).
...
registry, you may pull (download) the IBM Z Accelerated Serving for TensorFlow
container image with the following code block:
...
[IBM Z and LinuxONE Container Registry](https://ibm.github.io/ibm-z-oss-hub/containers/ibmz-accelerated-serving-for-tensorflow.html).
Release notes about a particular version can be found in this GitHub Repository
under releases
[here](https://github.com/IBM/ibmz-accelerated-serving-for-tensorflow/releases).

---

To remove the IBM Z Accelerated Serving for TensorFlow container image, please
follow the commands in the code block:
...
# TensorFlow Serving Usage <a id="tensorflow-serving"></a>

For documentation on serving models with TensorFlow Serving please visit the
official
...
IBM Z Accelerated Serving for TensorFlow container image follows IBM's train
...
[here](https://ibm.github.io/ibm-z-oss-hub/containers/ibmz-accelerated-for-tensorflow-serving.html).
...
You may have seen multiple TensorFlow Serving container images in IBM Z and
LinuxONE Container Registry, namely
[ibmz/tensorflow-serving](https://ibm.github.io/ibm-z-oss-hub/containers/tensorflow-serving.html)
and
...
it's users to transparently target the CPU. This container image contains the
optimizations referenced in this document.

## Q: Where can I run the IBM Z Accelerated Serving for TensorFlow container image?

You may run the IBM Z Accelerated Serving for TensorFlow container image on IBM
...
No. Installing newer or older version of TensorFlow Serving than what is
configured in the container will not target the IBM Integrated Accelerator for
...
IBM Z Accelerated Serving for TensorFlow will follow the
[semantic versioning guidelines](https://semver.org/) with a few deviations.
Overall, IBM Z Accelerated Serving for TensorFlow follows a continuous release
model with a cadence of 1-2 minor releases per year. In general, bug fixes will
be applied to the next minor release and not back ported to prior major or minor
...
Serving that are not likely backward compatible. Please refer to
[TensorFlow Serving guidelines](https://www.tensorflow.org/guide/versions) for
backwards compatibility across different versions of TensorFlow Serving.

## IBM Z Accelerated Serving for TensorFlow Versions

Each release version of IBM Z Accelerated Serving for TensorFlow has the form
MAJOR.MINOR.PATCH. For example, IBM Z Accelerated Serving for TensorFlow version
...
Feature releases for IBM Z Accelerated Serving for TensorFlow occur about every
6 months in general. Hence, IBM Z Accelerated Serving for TensorFlow 1.3.0 would
generally be released about 6 months after 1.2.0. Maintenance releases happen as
needed in between feature releases. Major releases do not happen according to a
fixed schedule.
...
can be found
[here](https://www14.software.ibm.com/cgi-bin/weblap/lap.pl?li_formnum=L-AVZW-TGVFHP)
